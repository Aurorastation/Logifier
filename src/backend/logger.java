...
public class logger {
    ...
    public static void populateLog(String[] searchWords) {
        ...
        try {
            ...
            if (Core.logFile == null) {
                final JFileChooser fc = new JFileChooser();
                fc.setDialogTitle("Select the log file you want to logify.");
                fc.setAcceptAllFileFilterUsed(false);
                FileNameExtensionFilter filter = new FileNameExtensionFilter("Log and TXT Files", "log", "txt");
                fc.addChoosableFileFilter(filter);
                int returnVal = fc.showOpenDialog(Core.menu);
                if (returnVal == JFileChooser.APPROVE_OPTION) {
                    Core.logFile = fc.getSelectedFile();
                    populateLog(null);
                } else {
                    System.exit(0); // we tried
                }
            }

            fileScan = new Scanner(Core.logFile);
